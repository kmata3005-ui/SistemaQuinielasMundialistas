...
{
    public async Task<(Usuario? Usuario, string? Error)> LoginAsync(
        ...
        string contrasena)
    {
        ...
        var usuario = await db.Usuarios.FirstOrDefaultAsync(
            x => x.NombreUsuario.ToLower() == id ||
                 x.Correo.ToLower() == id);

        if (usuario is null ||
            !PasswordService.Verify(contrasena, usuario.Contrasena))
        {
            return (null, "Usuario/correo o contraseña incorrectos.");
        }

        if (!usuario.Activo)
        {
            return (null, "Este usuario está desactivado. Contacte al administrador.");
        }
